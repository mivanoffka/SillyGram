...
async def get_user(manager: SillyManager, event: SillyEvent) -> Optional[SillyUser]:
    ...
    input_str = "?"
    try:
        input_str = await manager.get_input(event.user, SillyDefaults.Controls.USER_ID_INPUT_PROMPT)
        if not input_str:
            return None
        ...
        await manager.show_popup(event.user, SillyDefaults.Controls.ERROR_MESSAGE_TEMPLATE.format(
            SillyDefaults.Controls.USER_NOT_REGISTERED_ERROR_TEMPLATE.format(e)
        ))
        return None
